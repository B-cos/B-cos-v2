...
class Occlusion(ExplainerBase):
    def __init__(self, model, stride=32, ks=32, batch_size=8, only_positive=False):
        ...
        print(f"Using internal batch size of {batch_size} for Occlusion.")
    ...
    def make_masks(self, img):
        ...
        for idx in range(total):
            ...
            mask[
                max(0, (hpos + off) * stride[0] - ks2) : hpos * stride[0] + ks2,
                max(0, (wpos + off) * stride[1] - ks2) : wpos * stride[1] + ks2,
            ] = 0
        ...
        n_part = (participated.view(self.masks.shape[0], -1).sum(1))[
            :, None, None, None
        ]
    ...
    def attribute(self, img, target, return_all=False):
        ...
        org_out = self.model(
            img
        ).cpu()  # (1, n_classes). This is the prediction on the unperturbed input
        ...
        pert_out = torch.cat(
            [
                self.model(masked_input[idx * batch_size : (idx + 1) * batch_size]).to(
                    "cpu", non_blocking=True
                )
                for idx in range(int(np.ceil(len(masked_input) / batch_size)))
            ],
            dim=0,
        )
        ...
        diff = (
            (org_out - pert_out).clamp(0)
            if self.only_positive
            else (org_out - pert_out)
        )
        diff = diff[:, :, None, None]
